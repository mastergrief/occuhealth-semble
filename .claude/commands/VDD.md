...
**Agent strategy**: `DISCOVERY` in parallel (1x `Explore` & 1x `data`) → `Plan` (includes sizing + testable assertions) → `developer` (1 for small, 2+ sequential for medium/large) → [`review` + `browser`] in parallel → Both Pass → continue with remaining tasks (No need to plan twice unless fresh discovery or new [$ARGUMENTS])
...
**GATE**: Wait for both agents to complete. Do NOT proceed until all return.
